...
def regulatory_remark_for(goods_name: str, hs_code: str) -> str:
    ...
    if "火龙果" in goods_name or (hs_code and "0810" in hs_code):
        return "鲜火龙果属植物产品；R/S/检疫以总署及口岸动植检最新要求为准。本行由边民通规则生成，仅供参考。"
    if "木薯" in goods_name:
        return "块茎类可涉检疫与归类合并申报；以现场关员与检疫机构解释为准。本行由边民通规则生成。"
    if "牛" in goods_name or "活牛" in goods_name:
        return "活动物、鲜冷肉类监管条件更严，互市未必适用。本行由边民通规则生成，请优先咨询一般贸易/检疫要求。"
    return "税则号列、监管与检疫条件以《海关进出口税则》与现场关员认定为准。本行由边民通规则生成，仅供参考。"
