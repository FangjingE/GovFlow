...
class IntentService:
    ...
    - 互市/政务通分步填报相关表述 → 路由层先走政务答问并征求是否进入填报，确认后再切分步轨
    """

    _SOCIAL_HINT = re.compile(r"社保|医保|养老保险|断缴|补缴", re.I)
    _VAGUE_SOCIAL = re.compile(r"^(我想)?办社保$|办社保$|社保怎么办", re.I)
    # 与 knowledge_base/政务通 主题对齐（偏严，用于「仍在互市语境」判断）
    _ZWT_ROUTE = re.compile(
        r"政务通|边民互市|互市(?:进口|出口|申报|贸易|商品|限额)?|口岸互市|"
        r"跨境(?:边贸|农产品)|火龙果检疫|互市参考价",
        re.I,
    )
    _ZWT_SOFT = re.compile(
        r"政务通|边民互市|互市(?:进口|出口|申报|贸易|商品|限额)?|口岸互市|"
        r"跨境(?:边贸|农产品)|火龙果检疫|互市参考价|"
        r"进口商品|出口商品|(?:我要|想|要|准备).{0,8}(?:进口|出口)(?:商品|货物|东西)?",
        re.I,
    )
    _DENY_ZWT_START = re.compile(
    ...
    def hints_zwt_declaration_topic(self, user_text: str) -> bool:
        """是否与互市/政务通分步填报等话题相关（先答问，再征求是否进入填报）。"""
        t = user_text.strip()
        if len(t) < 3:
            return False
    ...
    def confirms_zwt_declaration_start(self, user_text: str) -> bool:
        ...
        if not t or len(t) > 36:
            return False
        if self.denies_zwt_declaration_start(t):
            return False
    ...
    def denies_zwt_declaration_start(self, user_text: str) -> bool:
        t = user_text.strip()
        if not t:
            return False
        return bool(self._DENY_ZWT_START.search(t))

    def wants_leave_zwt_for_gov(self, user_text: str) -> bool:
        """
        已在政务通分步轨时，是否应回到通用政务（显式强政务词且本句无互市/政务通主题）。
        """
        t = user_text.strip()
        if not t:
            return False
        if self.hints_zwt_declaration_topic(t):
            return False
        return bool(self._GOV_STRONG.search(t))
