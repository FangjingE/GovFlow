...
def test_zwt_start_only() -> None:
    r = client.post("/v1/zwt/turn", json={"start_only": True, "locale": "zh-CN"})
...
def test_zwt_happy_path_submit() -> None:
    a = client.post("/v1/zwt/turn", json={"message": ""})
    ...
    for msg in msgs:
        r = client.post("/v1/zwt/turn", json={"session_id": sid, "message": msg})
    ...
    c1 = client.post("/v1/zwt/turn", json={"session_id": sid, "message": "确认提交"})
    assert c1.status_code == 200
    c2 = client.post("/v1/zwt/turn", json={"session_id": sid, "message": "确认提交"})
    out = c2.json()
    assert out["kind"] == "submitted"
    assert out.get("submit_receipt")


def test_zwt_faq_rag_during_form() -> None:
    """申报途中咨询类问句走 RAG+LLM，不推进当前槽位（演示知识库/ mock 大模型）。"""
    a = client.post("/v1/zwt/turn", json={"message": ""})
    sid = a.json()["session_id"]
    for msg in ("进口", "火龙果"):
        r0 = client.post("/v1/zwt/turn", json={"session_id": sid, "message": msg})
    ...
    r2 = client.post("/v1/zwt/turn", json={"session_id": sid, "message": "30"})
...
def test_zwt_400_empty_message_with_session() -> None:
    a = client.post("/v1/zwt/turn", json={})
    sid = a.json()["session_id"]
    bad = client.post("/v1/zwt/turn", json={"session_id": sid, "message": "   "})
