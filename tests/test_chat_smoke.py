...
def test_index_ui() -> None:
    ...
    assert "政务通".encode("utf-8") in r.content
    assert b"zwt-aside" in r.content


def test_legacy_bmt_path_redirects_home() -> None:
    r = client.get("/bmt", follow_redirects=False)
    assert r.status_code == 302
    assert r.headers.get("location") == "/"


def test_chat_zwt_consent_then_form_and_can_leave_via_gov_topic() -> None:
    """互市类话题先答问并征求确认；用户同意后再出侧栏与填报轨。"""
    r1 = client.post("/v1/chat", json={"message": "我要进口商品"})
    assert r1.status_code == 200
    j1 = r1.json()
    assert j1["zwt_sidebar_visible"] is False
    ...
    j2 = r2.json()
    assert j2["zwt_sidebar_visible"] is True
    assert j2.get("zwt_form_preview")

    r3 = client.post("/v1/chat", json={"session_id": sid, "message": "办社保卡需要带什么材料"})
    assert r3.status_code == 200
    j3 = r3.json()
    assert j3["zwt_sidebar_visible"] is False


def test_chat_zwt_consent_decline() -> None:
    ...
    assert r2.status_code == 200
    j2 = r2.json()
    assert j2["zwt_sidebar_visible"] is False
